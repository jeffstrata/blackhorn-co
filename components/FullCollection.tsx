...
function SkullCard({ product, index }: { product: (typeof products)[0]; index: number }) {
  const handleInquire = () => {
    const subject = encodeURIComponent(`Inquiry: ${product.name}`)
    const body = encodeURIComponent(
      `Hi Blackhorn Co.,\n\nI'm interested in "${product.name}" (${product.finish === 'natural' ? 'Natural Bone' : 'Artist Painted'}) from your collection. Could you confirm availability and provide details?\n\nThank you!`
    )
  ...
  return (
    <article
      ...
      style={{ transitionDelay: `${(index % 3) * 60}ms`, boxShadow: '0 0 0 0 rgba(184,115,51,0)' }}
      ...
      <div className="p-5">
        ...
        <div className="flex items-center justify-between">
          ...
          {product.status === 'available' ? (
            ...
            <span className="font-body text-xs font-medium tracking-widest uppercase text-[#A8A29E]/50">Sold</span>
          )}
        </div>
      </div>
    </article>
...
export default function FullCollection() {
  ...
  return (
    <section ref={sectionRef} className="py-24 px-6 bg-[#0F0F0F] min-h-screen">
      <div className="max-w-7xl mx-auto">

        {/* Commission Banner */}
        <div className="reveal mb-12">
          <div className="relative border border-[#B87333]/30 bg-[#1A1A1A] overflow-hidden px-6 py-7 md:px-10 md:py-8 flex flex-col sm:flex-row items-center justify-between gap-6">
            <div className="absolute inset-0 pointer-events-none" style={{ background: 'radial-gradient(ellipse at 0% 50%, rgba(184,115,51,0.08) 0%, transparent 60%)' }} aria-hidden="true" />
            <div className="relative z-10 flex items-start gap-5">
              ...
              <div>
                <p className="font-display font-semibold text-[#FAF7F2] text-xl mb-1">Commission Custom Artwork</p>
                <p className="font-body text-[#A8A29E] text-sm leading-relaxed max-w-lg">
                  Want something truly one-of-a-kind? We work with Alberta&apos;s finest artists to hand-paint any skull to your vision. Starting at{' '}
        ...
        <div className="reveal mb-10 space-y-3">
          <div className="flex flex-wrap gap-2 items-center">
            <span className="font-body text-[10px] uppercase tracking-widest text-[#A8A29E]/50 w-full sm:w-auto sm:mr-2">Finish</span>
            {FINISH_FILTERS.map((f) => (
              <button
                ...
                className={`font-body text-xs font-medium tracking-widest uppercase px-4 py-2 transition-all duration-200 border ${
                  activeFinish === f
                    ? f === 'natural' ? 'bg-[#D4C5A9] border-[#D4C5A9] text-[#0F0F0F]' : 'bg-[#B87333] border-[#B87333] text-[#0F0F0F]'
          ...
          <div className="flex flex-wrap gap-2 items-center">
            <span className="font-body text-[10px] uppercase tracking-widest text-[#A8A29E]/50 w-full sm:w-auto sm:mr-2">Species</span>
        ...
        {/* Count */}
        <p className="font-body text-[#A8A29E]/50 text-xs tracking-widest uppercase mb-8 reveal">
        ...
        {filtered.length === 0 ? (
          <div className="text-center py-20">
            <p className="font-body text-[#A8A29E] mb-4">No pieces match that combination right now.</p>
            <button
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {paginated.map((product, index) => (
              <SkullCard key={product.id} product={product} index={index} />
            ))}
          </div>
        )}
            ...
            {Array.from({ length: totalPages }, (_, i) => i + 1).map((page) => (
              <button
                ...
                className={`w-9 h-9 font-body text-sm font-medium transition-all duration-200 ${
                  page === currentPage ? 'bg-[#B87333] text-[#0F0F0F]' : 'text-[#A8A29E] hover:text-[#FAF7F2] hover:bg-[#1A1A1A]'
                }`}
              >
                {page}
              </button>
